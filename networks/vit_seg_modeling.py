...
# set the dropout rate for the added dropouts in the encoder and decoder
my_dropout_rate = 0.18
...
#             query_weight = np2th(weights[pathhh]).view(self.hidden_size, self.hidden_size).t()
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        encoded1, attn_weights1 = self.encoder1(embedding_output)  # (B, n_patch, hidden)   *this is for mask
        encoded2, attn_weights2 = self.encoder2(embedding_output)  # (B, n_patch, hidden)   *this is for variance map
        return (encoded1, attn_weights1), (encoded2, attn_weights2), features
...
class Conv2dReLU(nn.Sequential):
    def __init__(
        ...
        if use_drop==True:
            drop = nn.Dropout(p = my_dropout_rate)
            super(Conv2dReLU, self).__init__(conv, bn, relu,drop)
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        (x1, attn_weights1), (x2, attn_weights2), features = self.transformer(x)  # (B, n_patch, hidden)
        x1 = self.decoder1(x1, features)
        x2 = self.decoder2(x2, features)
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            self.transformer.encoder1.encoder_norm.weight.copy_(np2th(weights["Transformer/encoder_norm/scale"]))
            self.transformer.encoder1.encoder_norm.bias.copy_(np2th(weights["Transformer/encoder_norm/bias"]))
            
            self.transformer.encoder2.encoder_norm.weight.copy_(np2th(weights["Transformer/encoder_norm/scale"]))
            self.transformer.encoder2.encoder_norm.bias.copy_(np2th(weights["Transformer/encoder_norm/bias"]))
